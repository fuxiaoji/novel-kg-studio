"""C21: local C15 graph reader under a reversed option order."""
...
VERSION = "c21-local-c15-reversed-option-order-v1"
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=BASE / "dqa_local_c21_permutation20")
    ...
    for novel in args.novels:
        ...
        for qi, q in enumerate(case["questions"][: args.max_questions or None]):
            ...
            if path.exists():
                continue
            permuted = {**q, "choices": list(reversed(q["choices"][:4]))}
            result = answer_graph(client, permuted, ctx, matrix)
            permuted_letter = result["selected_letter"]
            ...
            mapped = LETTERS[3 - LETTERS.index(permuted_letter)] if isinstance(permuted_letter, str) and permuted_letter in LETTERS else None
            gold = LETTERS[q["gold_index"]]
            result.update({"version": VERSION, "model": args.model, "thinking": "disabled", "external_api": False, "mask": "unmasked", "permutation": [3, 2, 1, 0], "permuted_letter": permuted_letter, "selected_letter": mapped, "novel": novel, "batch": "first10" if novel in FIRST10 else "second10", "qi": qi, "qid": q["qid"], "question": q["question"], "choices": q["choices"], "gold_letter": gold, "correct": mapped == gold})
